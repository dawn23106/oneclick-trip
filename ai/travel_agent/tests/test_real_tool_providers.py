...
def test_open_meteo_does_not_return_current_weather_for_distant_future_trip() -> None:
    client = httpx.Client(
        transport=httpx.MockTransport(
            lambda _: pytest.fail("distant future request must not call forecast API")
        )
    )
    start_date = date.today() + timedelta(days=30)

    result = OpenMeteoWeatherProvider(client=client).get_forecast(
        WeatherRequest(destination="成都", start_date=start_date)
    )

    assert result.success is True
    assert result.data_mode is ToolDataMode.FALLBACK
    assert result.data["daily"] == []
    assert "超出15日天气预报范围" in result.data["summary"]
