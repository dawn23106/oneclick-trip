...
from app.jobs import InMemoryAgentJobStore, RedisAgentJobStore
...
def create_app(
    ...
    job_store = InMemoryAgentJobStore()
    if not isolated_mode and configured_settings.redis_url:
        ...
        try:
            ...
            redis_job_store.recover_interrupted()
            job_store = redis_job_store
        except Exception:
            redis_job_store.close()
    ...
    if not isolated_mode:
        if not configured_settings.use_external_infrastructure:
            # The local development profile must still expose a usable knowledge
            # pipeline. Hash embeddings and embedded Chroma require no model
            # download or separately running vector database.
            embedding_function = HashEmbeddingFunction()
        knowledge_base = ChromaTravelKnowledgeBase(
            ...
            server_url=(
                configured_settings.chroma_server_url
                if configured_settings.use_external_infrastructure
                else None
            ),
        )
        ...
        infrastructure_status["chroma"] = (
            "configured" if configured_settings.use_external_infrastructure else "local"
        )
    ...
    async def lifespan(application: FastAPI) -> AsyncIterator[None]:
        ...
        if knowledge_base is not None:
            try:
                ...
                infrastructure_status["chroma"] = (
                    "ok" if configured_settings.use_external_infrastructure else "local"
                )
            except Exception:
    ...
    application.state.agent_jobs = job_store
