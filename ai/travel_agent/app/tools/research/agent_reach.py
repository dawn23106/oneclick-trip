...
class AgentReachWebSearch:
    ...
    def __init__(
        ...
        *,
        ...
        self._runner = runner or _run_command
        self._cache = cache
        self._fallback_enabled = _fallback_enabled() if fallback_enabled is None else fallback_enabled
    ...
    def search(self, query: str, *, limit: int = 5) -> ToolResult:
        ...
        if (
            ...
            and not normalized_query.isascii()
            and self._runner is _run_command
            and self._executable is None
        ):
            fallback = _jina_bing_search(
                normalized_query,
                limit,
                time.monotonic(),
                enabled=True,
...
def _fallback_enabled() -> bool:
    return os.getenv("AGENT_REACH_FALLBACK_ENABLED", "false").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _bing_rss_search(
    ...
    *,
    ...
    """Use Bing's public RSS feed when Exa's shared MCP quota is unavailable."""
    if not (_fallback_enabled() if enabled is None else enabled):
        return None
    try:
        endpoint = "https://www.bing.com/search?" + urlencode(
            {"format": "rss", "q": query}
        )
        request = Request(endpoint, headers={"User-Agent": "oneclick-trip/1.0"})
...
def _jina_bing_search(
    ...
    *,
    ...
    if not (_fallback_enabled() if enabled is None else enabled):
        return None
    try:
        endpoint = "https://r.jina.ai/http://cn.bing.com/search?" + urlencode(
            {"q": query}
        )
        request = Request(endpoint, headers={"User-Agent": "oneclick-trip/1.0"})
    ...
    items = []
    seen_urls = set()
    skip_hosts = ("bing.com", "microsoft.com", "mm.bing.net", "miit.gov.cn", "mps.gov.cn")
    for match in re.finditer(r"\[([^\]]{2,160})\]\((https?://[^)]+)\)", text):
        ...
        if (
            ...
            or any(skip_host in host for skip_host in skip_hosts)
        ):
            continue
        ...
        items.append(
            ResearchItem(
                title=title,
                url=url,
                summary=_summary_near(text, match.end()),
    ...
    return ToolResult(
        success=True,
        source="agent-reach/jina-bing",
        data_mode=ToolDataMode.REALTIME,
        confidence=_result_confidence(items),
        data={
            "query": query,
            "provider": "jina-bing-fallback",
            "count": len(items),
            "items": [item.model_dump(mode="json") for item in items],
            "elapsed_ms": round((time.monotonic() - started) * 1000),
        },
...
def _summary_near(text: str, start: int) -> str:
    snippet = text[start : start + 500]
    snippet = re.sub(r"!\[[^\]]*\]\([^)]+\)", "", snippet)
    snippet = re.sub(r"\[([^\]]+)\]\([^)]+\)", r"\1", snippet)
    return _plain_markdown_text(snippet)[:300]


def _jina_fetch_pages(
    urls: list[str],
    max_characters: int,
    started: float,
    *,
    ...
    """Read public pages through Jina Reader when Exa fetch is unavailable."""
    if not (_fallback_enabled() if enabled is None else enabled):
        return None
    pages = []
    for url in urls:
        try:
            request = Request(
                ...
                headers={"User-Agent": "oneclick-trip/1.0"},
            )
            with urlopen(request, timeout=25) as response:
                text = response.read().decode("utf-8", errors="replace")
            ...
            content = (
                text.split(content_marker, 1)[1].strip()
    ...
    if not pages:
        return None
    found = {_canonical_url(page.url) for page in pages}
    return ToolResult(
        success=True,
        source="agent-reach/jina-fetch",
        data_mode=ToolDataMode.REALTIME,
        confidence=0.7,
        data={
            "count": len(pages),
            ...
            "missing_urls": [url for url in urls if _canonical_url(url) not in found],
            "partial": len(pages) < len(urls),
            "elapsed_ms": round((time.monotonic() - started) * 1000),
        },
    )
