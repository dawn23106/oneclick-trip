...
class OpenMeteoWeatherProvider:
    ...
    def get_forecast(self, request: WeatherRequest) -> ToolResult:
        ...
        if request.start_date and request.start_date > latest_forecast_date:
            return ToolResult(
                success=True,
                source="open-meteo",
                ...
                confidence=1.0,
                data={
                    "data_mode": ToolDataMode.FALLBACK.value,
                    "source": "open-meteo",
                    "destination": request.destination,
                    "current": {},
                    "daily": [],
                    "summary": (
                        f"{request.destination}{request.start_date.isoformat()} "
                        "超出15日天气预报范围，请在临近出发时重新查询"
                    ),
                },
            )
        try:
