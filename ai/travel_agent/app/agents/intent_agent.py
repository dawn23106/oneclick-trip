...
class LangChainIntentAgent:
    ...
    def classify(self, query: str, *, context: IntentContext | None = None) -> IntentDecision:
        """同步调用大模型并将输出校验为严格的 IntentDecision。"""
        ...
        return [
            SystemMessage(
                content=AGENT_SECURITY_POLICY + (
                    ...
                    f"今天的日期是 {date.today().isoformat()}，今天/明天/后天及节日日期必须以这个日期为基准解析。"
                    "元旦、春节、清明、劳动节、端午、中秋、国庆等节日属于有效日期表达；"
                    "模型可以识别语义，但最终日期由代码按公历、农历或节气规则校正。"
...
class RuleBasedIntentAgent:
    ...
    def _detect_intent(self, text: str) -> Intent:
        ...
        if any(place in text for place in self.DESTINATION_NAMES) and any(
            ...
            return Intent.TRIP_PLAN
        # "去成都"、"到西安"、"飞三亚" — bare destination requests → trip plan
        if any(place in text for place in self.DESTINATION_NAMES) and re.search(
            r"(?:^|[，,。；;]\s*)(?:去|到|前往|飞|直飞|出发去|准备飞|打算飞)\s*[^\s，,。；;]+",
            text,
        ):
            return Intent.TRIP_PLAN
    ...
    def _extract_entities(self, text: str, intent: Intent) -> TravelEntities:
        ...
        days_match = next(
            (
                candidate
                for candidate in re.finditer(rf"{duration_number}\s*(?:天|日)游", text)
                if text[max(0, candidate.start() - 1):candidate.start()] not in {"至", "到", "-", "~"}
            ),
            None,
        )
