...
// ===== AI 行程版本管理 =====
export function fetchAiTripPlans(params = {}) {
  const query = new URLSearchParams(params).toString()
  return request(`${BASE}/ai-trip-plans?${query}`)
}

export function fetchAiTripPlan(id) {
  return request(`${BASE}/ai-trip-plans/${id}`)
}

export function deleteAiTripPlan(id) {
  return request(`${BASE}/ai-trip-plans/${id}`, { method: 'DELETE' })
}

export function restoreAiTripPlan(id) {
  return request(`${BASE}/ai-trip-plans/${id}/restore`, { method: 'POST' })
}
