...
function normalizeRule(data) {
  return {
    ...
    tripStatus: data.tripStatus || 'PLANNING',
...
function normalizeAi(data, recordId) {
  ...
  return {
    ...
    summary: Array.isArray(plan.assumptions) ? joinSentences(plan.assumptions) : (plan.summary || '由 AI 多阶段规划生成。'),
    days: days.length,
...
Page({
  ...
  async loadTrip() {
    ...
    try {
      ...
      const trip = this.data.type === 'AI' ? normalizeAi(data, this.data.id) : normalizeRule(data)
      trip.metaLine = [trip.startDate ? dateLabel(trip.startDate) : '日期待定', `${trip.days || 0} 天`, `${trip.people || 1} 人`].join(' · ')
      this.setData({ trip, activeDay: 0, tripStatus: trip.tripStatus || 'PLANNING' })
  ...
  async toggleStatus() {
    if (this.data.togglingStatus) return
    const currentStatus = this.data.tripStatus || 'PLANNING'
    const newStatus = currentStatus === 'COMPLETED' ? 'PLANNING' : 'COMPLETED'
    this.setData({ togglingStatus: true })
    try {
      if (this.data.type === 'AI') {
        await api.updateAiTripStatus(this.data.id, newStatus)
      } else {
        await api.updateTripStatus(this.data.id, newStatus)
      }
      this.setData({ tripStatus: newStatus })
      wx.showToast({ title: newStatus === 'COMPLETED' ? '已标记完成' : '已重新激活', icon: 'success' })
    } catch (error) {
