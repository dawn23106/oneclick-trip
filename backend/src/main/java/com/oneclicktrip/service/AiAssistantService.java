...
public class AiAssistantService {
    ...
    private final Set<String> finalizedRuns = ConcurrentHashMap.newKeySet();
    ...
    public JsonNode job(String runId, Long authenticatedUserId) {
        // 轮询时再次校验任务归属，避免仅凭 runId 越权读取其他用户的 Agent 结果。
        ...
        if ("COMPLETED".equals(status) && job.path("result").isObject()) {
            ...
            if (completed == null) {
                completed = recoverContext(job, requestingUserId);
            }
            if (completed != null && finalizedRuns.add(runId) && !agentRunLogService.exists(runId)) {
            ...
            if (failed == null) {
                failed = recoverContext(job, requestingUserId);
            }
            if (failed != null && finalizedRuns.add(runId) && !agentRunLogService.exists(runId)) {
                agentRunLogService.record(
                        runId,
                        failed.userId(),
    ...
    private AsyncCallContext recoverContext(JsonNode job, Long userId) {
        // Java 服务重启后内存映射会丢失，此处根据 FastAPI 作业结果恢复持久化所需上下文。
        String conversationId = job.path("conversation_id").asText();
        if (conversationId.isBlank()) {
            conversationId = job.path("result").path("conversation_id").asText();
        }
        if (conversationId.isBlank()) {
            return null;
        }
        ...
        try {
            conversation = conversationService.findOwned(userId, conversationId);
        } catch (RuntimeException ignored) {
            // The Agent result is still queryable even if its chat was deleted
            // while Python was running or while the Java service restarted.
            conversation = null;
        }
        return new AsyncCallContext(userId, conversationId, "", conversation);
    }
    ...
    public AiChatResponse resume(AiResumeRequest request, Long authenticatedUserId) {
        // resume 只用于继续已被 LangGraph 人工确认点暂停的预订流程。
        Long userId = requireAuthenticatedUser(authenticatedUserId);
    ...
    private AiChatResponse toResponse(JsonNode state) {
        // 将 FastAPI 的完整 TravelState 收敛为前端稳定使用的业务响应。
